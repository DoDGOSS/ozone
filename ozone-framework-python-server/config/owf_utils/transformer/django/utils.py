import re
import copy
from typing import Dict, Callable, Optional, Union
from django.http import HttpRequest, HttpResponse

from .settings import django_settings

json = django_settings.JSON_MODULE


def swap_data_keys(data: Dict) -> Optional[Dict]:
    swap_map = {
        'data': 'results',
        'results': 'count'
    }
    results = copy.deepcopy(data)
    for k, v in swap_map.items():
        if v in data:
            results[k] = data[v]
            del results[v]

    return results


def to_camel_case(snake_case_str: str) -> str:
    components = snake_case_str.split('_')
    titled_components = ''.join(x.title() for x in components[1:])

    return f'{components[0]}{titled_components}'


def to_snake_case(camel_case_str: str) -> str:
    return re.sub('([A-Z])([a-z0-9]+)', r'_\1\2', camel_case_str).lower()


def nested_case_transform(data: Dict, to_case_func: Callable) -> Optional[Dict]:
    transformed_data = {}

    if data is None:
        return

    if isinstance(data, list):
        new_data = []
        for obj in data:
            if isinstance(obj, dict):
                new_data.append(nested_case_transform(obj, to_case_func))
            else:
                new_data.append(obj)

        return new_data

    if isinstance(data, dict):
        for key, value in data.items():
            transformed_key = to_case_func(key)
            if not isinstance(value, dict) and not isinstance(value, list):
                transformed_data[transformed_key] = value

            if isinstance(value, list):
                new_value = []
                for obj in value:
                    if isinstance(obj, dict):
                        new_value.append(nested_case_transform(obj, to_case_func))
                    else:
                        new_value.append(obj)

                transformed_data[transformed_key] = new_value

            if isinstance(value, dict):
                transformed_data[transformed_key] = nested_case_transform(value, to_case_func)

    return transformed_data


def nested_camel_case_transformer(data: Dict) -> Dict:
    return nested_case_transform(data, to_camel_case)


def nested_snake_case_transformer(data: Dict) -> Dict:
    return nested_case_transform(data, to_snake_case)


def json_loads(request: HttpRequest) -> Union[dict, list]:
    if hasattr(request, 'json'):
        return request.json

    return json.loads(request.body)


def is_json_possible(request: HttpRequest) -> bool:
    return bool(
        request.method not in ('GET', 'HEAD') and
        request.content_type == 'application/json' and
        request.body
    )


def is_json_content(response: HttpResponse) -> bool:
    return bool(
        response and
        response.get('Content-Type') == 'application/json' and
        response.content
    )
