...
import { asStyles, BoundingBox, empty, split } from "./util/BoundingBox";
...
export class MosaicRoot<T extends MosaicKey> extends React.PureComponent<MosaicRootProps<T>> {
    ...
    render() {
        ...
        return <div className="mosaic-root">{this.renderRecursively(root, empty(), [])}</div>;
    }
    ...
    private renderRecursively(
        ...
        if (isParent(node)) {
            ...
            return (
                <div key={node} className="mosaic-tile" style={{ ...asStyles(boundingBox) }}>
