...
//        personWidgetDefinitionService.create(generatePWDPostParamsA())
        DomainBuilders.createPersonWidgetDefinition(user1, widgetDef)

        when:
        def result = personWidgetDefinitionService.show([guid: widgetDef.widgetGuid, personId: accountService.getLoggedInUser().id])
    ...
    void testList() {
        ...
        def widget_1 = createWidgetDefinition(WidgetDefinitionPostParams.generatePostParamsA())
        def widget_2 = createWidgetDefinition(WidgetDefinitionPostParams.generatePostParamsC())

        DomainBuilders.createPersonWidgetDefinition(user1, widget_1)
        DomainBuilders.createPersonWidgetDefinition(user1, widget_2)
