...
. Before starting the upgrade, backup the corresponding database previously used with OWF 7.17.1.0 as well as all custom override configuration files (In Tomcat, they are normally located in the `{path-classpath}/` directory)
+
...
. Make any configuration changes needed in `{file-app-yml}` configuration file, located in the bundle in the `{path-ozone}/` directory.
. Completing the upgrade
.. Reconfigure and re-apply customizations in the `{path-classpath}/` directory
...
.. Copy any custom certificates from previous build to OWF v{version-ozone}. Usually this is found in `{path-tomcat-certs}/`.
.. Upgrade is now complete


== Connecting to OMP (Marketplace)

In order to the connect your OMP Marketplace (v7.17.X.X) with OWF v{version-ozone}, you must modify the following files:
