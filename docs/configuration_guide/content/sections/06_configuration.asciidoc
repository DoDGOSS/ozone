...
OWF offers a variety of custom configuration options in the externalized `{file-app-yml}` configuration file, located in the bundle in the `{path-ozone}/` directory. Once changes are made, restart the system to apply the changes.
