...
The OWF Bundle is shipped with Tomcat v{version-tomcat} and requires Java 8 or higher. If running OWF with a Web server other than Tomcat, see that Web server's documentation for any additional requirements.
...
owf.war::
This file, located in the `{path-webapps}/` directory, contains the components which make up the OWF application. Whether a user logs in and accesses the framework, or an administrator logs in to modify preferences, the `owf.war` is the application that launches those pages to the browser.
...
Included within the OWF bundle and framework `{path-ozone}/` is the default `security.xml` Spring configuration file, also within `{path-ozone-security}/` are additional example configurations files.
