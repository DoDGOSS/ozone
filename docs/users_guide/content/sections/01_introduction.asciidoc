...
NOTE: Any Java Servlet-compliant application server can host the Ozone application. Additionally, the CAS server and LDAP components are examples of possible security integrations -- the Pluggable Security component acts as an adapter to integrate with many different enterprise security solutions.
